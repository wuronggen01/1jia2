...
var accessTokenStoreKey = burl + "-" + appID + "other.platform.access.token";
var accessToken;
...
$(document).ready(function(tokenObject) {

    if (window.localStorage) {
        try {
            var oldToken = JSON.parse(window.localStorage.getItem(accessTokenStoreKey));
            if (oldToken) {
                accessToken = oldToken.token;
                $.jsonrpc("other.checkLoginStatus", [], function(error, result) {
                    if (!result) {
                        accessToken = null;
                        window.localStorage.removeItem(accessTokenStoreKey);
                    }
                });
            } else {
                window.localStorage.removeItem(accessTokenStoreKey);
...
$.extend({
    ...
    jsonrpcSaveAccessToken: function(tokenObject) {
        if (window.localStorage) {
            if (tokenObject) {
                window.localStorage.setItem(accessTokenStoreKey, JSON.stringify(tokenObject));
            } else {
                window.localStorage.removeItem(accessTokenStoreKey);
            }
        }
    ...
    jsonrpc: function(methodname, params, callback, options) {
        ...
        if (options) {
            ...
            baseAppID = options.baseAppID ? options.baseAppID : baseAppID;
            baseAppSecret = options.baseAppSecret ? options.baseAppSecret : baseAppSecret;
        ...
        var jsonrpcExecute = function() {
            ...
            if ('string' == typeof methodname) {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    headers: {
                        "other-app-id": (baseAppID ? baseAppID : ""),
